...
namespace SanDX {
	template<typename T, typename std::enable_if<std::is_base_of<Material, T>::value>::type*>
	inline bool MaterialLoader::Load(std::weak_ptr<T>& outMaterial) {
		...
		std::shared_ptr<Material> newMaterial = std::make_shared<T>();
